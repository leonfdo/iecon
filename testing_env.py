...
def map_grid(n):
    file_name="C:\\Users\\leonf\\OneDrive\\Documents\\SUTD\\Single_Multi_drone\\Test_maps\\obs"+str(n)+".csv"
...
class drone_env(gym.Env):
    ...
    def take_pic(self,model_name,steps):
        #image = Image.fromarray(self.obs_cells,mode="L")
        name = f"arrays\model_{model_name}_step_{steps}.csv"
        np.savetxt(name,self.obs_cells,delimiter=",", fmt="%d")
        #image.save(name)
    ...
    def step(self, action):
        ...
        if self.steps_taken%10==0:
            self.take_pic(self.random_num, self.steps_taken)
